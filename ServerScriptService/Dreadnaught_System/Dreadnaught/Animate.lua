...
local RightShoulder = Torso:WaitForChild("Right Shoulder",10)
local LeftShoulder = Torso:WaitForChild("Left Shoulder",10)
...
local animNames = { 
	idle = 	{	
		{ id = "http://www.roblox.com/asset/?id=0", weight = 9 },  --180435571
				{ id = "http://www.roblox.com/asset/?id=0", weight = 1 }, --180435792
				{ id = "http://www.roblox.com/asset/?id=0", weight = 1 }
			},
	walk = 	{ 	
		{ id = "http://www.roblox.com/asset/?id=17375446412", weight = 10 } 
			}, 
	run = 	{
		{ id = "http://www.roblox.com/asset/?id=17375446412", weight = 10 }  --run.xml
			}, 
	jump = 	{
				{ id = "http://www.roblox.com/asset/?id=125750702", weight = 10 } 
			}, 
	fall = 	{
		{ id = "http://www.roblox.com/asset/?id=0", weight = 10 }  --180436148
			}, 
	climb = {
...
function playAnimation(animName, transitionTime, humanoid) 
	...
	if animName == "idle" then
		if script.Parent.Settings.Combat.Target.Value then
			idx = 3
		...
		--[[for _,Child in pairs(script.Parent.Humanoid:GetPlayingAnimationTracks()) do
			if Child.Name ~= "Animation3" then
				Child:Stop()
			end
		end]]
	end
...
local airTimeReq = 0.7
local timeInAir = 0
local dreadModel = script.Parent
local char = dreadModel.Parent
local rightLeg = nil
local leftLeg = nil
local allow = false
local ArmorTable = {
...
for _,v in pairs(dreadModel:GetChildren()) do
	if not v:IsA('Model') then continue end
	if v.Name == "Right Leg" then
		rightLeg = v:WaitForChild('RightLeg'):WaitForChild('Att')
	elseif v.Name == "Left Leg" then
		leftLeg = v:WaitForChild('LeftLeg'):WaitForChild('Att')
	end
end
...
Humanoid:GetPropertyChangedSignal('FloorMaterial'):Connect(function()
	...
	elseif allow then
		local t = tick() - timeInAir
		if t <= airTimeReq then return end
		dreadModel.PrimaryPart.Metal:Play()
		local boom = Instance.new("Explosion")
		boom:SetAttribute('Ignore',true)
		boom.BlastPressure = 10
		boom.BlastRadius = 15*t
		boom.DestroyJointRadiusPercent = 0
		boom.ExplosionType = Enum.ExplosionType.NoCraters
		boom.Visible = false
		boom.Parent = workspace
		boom.Position = leftLeg.WorldPosition
		...
		boom.Hit:Connect(function(hit,distance)
			...
			local dam = math.clamp(distance/boom.BlastRadius,0.001,1)*3
			if hum then
				if hum.Parent:FindFirstChild('Dreadnaught') then
					if hum.Parent.Dreadnaught == dreadModel then return end
					hum.Parent.Dreadnaught.VecHealth.Value -= dam*5
				...
				if hit.Parent == dreadModel then return end
		...
		boom2:SetAttribute('Ignore',true)
		boom2.BlastPressure = 10
		boom2.BlastRadius = 15*t
		boom2.DestroyJointRadiusPercent = 0
		boom2.ExplosionType = Enum.ExplosionType.NoCraters
		boom2.Visible = false
		boom2.Parent = workspace
		boom2.Position = rightLeg.WorldPosition
		game.Debris:AddItem(boom2,1)
		boom2.Hit:Connect(function(hit,distance)
			local hum = hit.Parent:FindFirstChild('Humanoid')
			if hum and hum.Parent.Name == "Arms" then
				hum = hum.Parent.Parent:FindFirstChild('Humanoid')
			end
...
function onRunning(speed)
	Humanoid:SetStateEnabled(Enum.HumanoidStateType.Freefall,false)
	Figure.Settings.General.Walking.Value = speed > .01
	if speed > .01 then
		playAnimation("walk", 0.1, Humanoid)
		local s = speed/7
		if Figure:GetAttribute('Reversing') == true then
			s = 4
		end
		if math.abs(s - previousS) > 0.1 then
			setAnimationSpeed(s)
		end
