...
local SOUND_DATA : { [string]: {[string]: any}} = {
	...
	Died = {
		SoundId = "rbxassetid://106938261070826",
	},
	FreeFalling = {
	...
	Jumping = {
		SoundId = "rbxassetid://6769750305",
	},
	Landing = {
		SoundId = "rbxassetid://110067319414698",
	},
	Running = {
		SoundId = "rbxassetid://83542378456462", 
		Looped = true,
...
local function initializeSoundSystem(instances)
	...
	local stateTransitions: {[Enum.HumanoidStateType]: () -> ()} = {
		...
		[Enum.HumanoidStateType.Landed] = function()
			...
			--if verticalSpeed > 75 then
				sounds.Landing.Volume = math.clamp(map(verticalSpeed, 50, 100, 0, 1), 1.5, 3)
				sounds.Landing.PlaybackSpeed = 0.85
	...
	local loopedSoundUpdaters: {[Sound]: (number, Sound, Vector3) -> ()} = {
		...
		[sounds.Running] = function(dt: number, sound: Sound, vel: Vector3)
			sound.Playing = vel.Magnitude > 5 and humanoid.MoveDirection.Magnitude > 0.5
			...
			if sound.Playing then
				...
				sound.PlaybackSpeed = humanoid.WalkSpeed / baseSpeed
			end
		end,
	}
